...
# Verify the clear actually landed before injecting, rather than trusting a bare sleep:
# poll (bounded ~6s) for a pane that is BOTH static (two captures ~0.8s apart match) AND
# still shows the Claude Code REPL fingerprint (the input prompt / bypass-permissions footer).
# If /clear hadn't finished, injecting onto a not-yet-cleared session could re-trigger the
# very compact we're avoiding. WARN + proceed on timeout so a UI change never wedges the run.
# (Pattern hardened in lockstep with cairn's nightly — a0b25f5.)
cleared=""
for _ in 1 2 3 4 5 6 7 8; do
  c1=$(tmux capture-pane -t "$SESSION" -p -S -8 2>/dev/null)
  sleep 0.8
  c2=$(tmux capture-pane -t "$SESSION" -p -S -8 2>/dev/null)
  if [ "$c1" = "$c2" ] && echo "$c2" | grep -qiE "bypass permissions|for shortcuts|❯"; then
    cleared=1; break
  fi
done
[ -n "$cleared" ] || echo "WARN: could not confirm /clear landed within ~6s; injecting anyway."
